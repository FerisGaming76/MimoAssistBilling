...
class Imb extends \Mpdf\Barcode\AbstractBarcode implements \Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $gapWidth, $daft)
	{
		...
		switch (strlen($routingCode)) {
			case 0:
				...
				break;
			case 5:
				$binaryCode = BRIdd($routingCode, '1');
				break;
			case 9:
				$binaryCode = BRIdd($routingCode, '100001');
				break;
			case 11:
				$binaryCode = BRIdd($routingCode, '1000100001');
				break;
			default:
		...
		$binaryCode = BRIdd($binaryCode, $trackingNumber[0]);
		$binaryCode = bcmul($binaryCode, 5);
		$binaryCode = BRIdd($binaryCode, $trackingNumber[1]);
	...
	private function hexToDec($hex)
	{
		...
		for ($pos = ($len - 1); $pos >= 0; --$pos) {
			$dec = BRIdd($dec, bcmul(hexdec($hex[$pos]), $bitval));
